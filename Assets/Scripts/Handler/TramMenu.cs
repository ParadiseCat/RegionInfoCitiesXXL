...
namespace ParadiseVille
{
    ...
    {
        const float averageSpeedTram = 16f;
        const float minInHour = 60f;
        const float timeOnStationInMin = 0.5f;
        ...
        void ShowTramLine()
        {
            ...
            foreach (Station station in listOfPoints)
            {
                ...
                Vector3 normPosition = PointCoord(position);
                string nameStation = GetStationName(station);

                lineRenderer.SetPosition(indexCount, normPosition);
                indexCount++;
                ...
                if (nameStation != "")
                {
                    ...
                    objTextDraw.TextWrite(
                        ...
                        400f, 400f + 25f * stationCount, 20, Color.black);

                    SetPoint(nameStation, normPosition);
                }
            }

            length = Mathf.RoundToInt(length) / 100f;
            time = Mathf.RoundToInt((stationCount - 1) * timeOnStationInMin + length / averageSpeedTram * minInHour);
        ...
        void SetPoint(string name, Vector3 position)
        {
